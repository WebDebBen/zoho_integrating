...
ZOHO.CREATOR.init().then(async function (data) {
    ...
    var user_email = queryParams["loginUserEmail"];
    ...
    recommendations?.length && recommendations?.map(recom => {
        list += `
        <div class="list-div">
            <img class="icon" src=${getDomain() + recom?.Attack_Vector_Active_Logo_URL} />
    ...
    body.addEventListener("click", function (e) {
        if(e.target.getAttribute("id") === "view-detail-btn"){
            const dotID = e.target.getAttribute("dot-id");
            const vectorID = e.target.getAttribute("vector-id");
                window.open(getDomain() +`/peter_bluewire/customer-portal/#Page:Recommendation_Engine_S?loginUserEmail=`+queryParams["loginUserEmail"]+`&selectedDOTid=`+dotID+`&selectedAttackVectorID=`+vectorID, "_parent");
        }
    })
    const viewAllBtn = document.getElementById("view-all-btn");
    viewAllBtn?.addEventListener('click', () => {
        window.open(getDomain() +"/peter_bluewire/customer-portal/#Page:Recommendation_Engine_S?loginUserEmail=" + user_email,"_parent");
    });
})