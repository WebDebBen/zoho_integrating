...
ZOHO.CREATOR.init().then(async function (data) {
    ...
    recommendations?.length && recommendations?.map(recom => {
        const url = `https://creatorapp.zoho.com/peter_bluewire/customer-portal/#Page:Recommendation_Engine_S?loginUserEmail=${queryParams["loginUserEmail"]}&&selectedDOTid=${recom?.DOT?.ID}&&selectedAttackVectorID=${recom?.Reptile_Index_Theory_Attack_Vector?.ID}`;
        console.log(url)
        list += `
        <div class="list-div">
            ...
            <div class="content-div">
                ...
                <a 
                    ...
                    href="#" 
                    dot-id=${recom?.DOT?.ID}  
                    onclick="window.open(${url}, "_parent")"
