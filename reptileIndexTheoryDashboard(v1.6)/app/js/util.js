...
$(document).ready(function(){
    max_date = $("#chartslider_input").attr("data-slider-max");
    ...
    $("#chart_end_date").html(getDateFromPoint(max_date));
...
function getDateFromPoint(point ){
    ...
    var d = now_date - (max_date - point) * 1 * 24 * 3600 * 1000;
    d = new Date(d); 
...
function displayAttackValue(attack_value, attack_vector, total_value ){
    ...
    for (var i = 0; i < attack_vector.length; i++ ){
        ...
        $("<img>").attr("src", getDomain() + item.Inactive_Logo )
                .attr("data-active", getDomain() + item.Active_Logo_URL )
...
function setRepDifference(rep_data ){
    var now_date = getDateFromPoint(max_date);
    var sub_date = getDateFromPoint(max_date - 7);
